...
const SearchBar = (props: Props)=>{
  ...
  const search = async (event: any) => {
    ...
    let payload = '';
    payload  = category !== '' || category !== SearchCategories[0] ? payload + 'category=' + category + '&' : payload + '';
  ...
  return (
    <div className=" w-full flex">
        ...
        <option key={index} className=" z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">{item}</option>
        ))}
        </Form.Select>
        <div className=" relative w-full">
            <input type="search" onChange={handleKeyword} id="search-dropdown" className=" block p-2.5 w-full z-20 text-sm text-gray-900 bg-gray-50 rounded-r-lg border-l-gray-50 border-l-2 border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-l-gray-700  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-blue-500" placeholder="Search Mockups, Logos, Design Templates..." />
